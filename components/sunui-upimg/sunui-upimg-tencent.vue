...
<script>
	...
	export default {
		...
		methods: {
			...
			uploadAudio(e) {
				uploadAudio(this, e, this.audio, 1);
			},
			chooseImage(count) {
	...
	const uploadAudio = async (_this, configs, data, j) => {
		console.log("uploadAudio:", configs, data, j)
		// 腾讯云配置
		let cosConfig = {
		...
		let opt = {
			...
			Key: configs.path + configs.photoType + "audio.mp3",
			// Key: configs.audioKey, //configs.path + configs.photoType + "123123.wav",
			Body: data,
